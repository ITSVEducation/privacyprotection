...
| ファイル（マスクの判定、「確認なしで即変換」OFF） | ファイルごとに解析をワーカースレッドで実行 → プレビュー（§4.2）→ 確定したものだけマスクし、ファイルごとにレポートを表示。複数ファイルは1件ずつ順に処理する |
| ファイル（マスクの判定、「確認なしで即変換」ON） | 解析をワーカースレッドで実行し、プレビューを飛ばして即マスクし、ファイルごとにレポートを表示 |
